...
def read_T(id):
    meta_data = load_yaml(id)  #meta_data contains data for train,test, and val patterns including pathways which need to be changed
...
def get_dataset(config, key="TRAIN"):
    ...
    files = open(config['FILE_LIST']).readlines() #files_clean.txt
    files = [s.replace('\n','') for s in files]
    ...
    N    = X.shape[0]
    X_ = np.zeros((N,config['CENTER_DIMS'],config['CENTER_DIMS']))
    Y_ = np.zeros((N,config['CENTER_DIMS'],config['CENTER_DIMS']))
